...
Hotový prototyp jsem po otestování podrobil měření spotřeby, ve všech možných režimech. Měření probíhalo při pokojové teplotě 22 \degree C a při výpočtech spotřeby ve dnech, počítám s nejslabší možnou autobaterií 50 Ah a dny jsou zaokrouhlovány vždy dolů, na celý jeden den. Aktuálně zařízení vydrží připojené na nedobíjenou autobaterii přibližně 30 dnů, při správné implementaci zdrojových kódů akcelerometru, do zdrojových kódů řídící jednotky, které jsem již zhotovill, je možné dosáhnout výdrže až 200 dní. V případě vybité baterie, by bylo vhodné uživatele notifikovat o tomto stavu.
